...
const input = readFileSync(resolve(process.env.INPUT || 'input.txt')).toString();
...
const fetchData = async (ticker, page_token) => {
    ...
    const res = await fetch(`https://data.alpaca.markets/v2/stocks/${ticker}/bars?` + new URLSearchParams(paramsInit), {
        headers: new Headers({
